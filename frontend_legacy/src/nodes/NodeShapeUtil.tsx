...
function NodeShape({ shape }: { shape: NodeShape }) {
	...
	// Get model name for image/video nodes
	let headingText = nodeDefinition.heading ?? nodeDefinition.title
	if (shape.props.node.type === 'imageGeneration' && 'model' in shape.props.node) {
		const MODELS = [
			{ id: 'flux-pro', name: 'Flux Pro 1.1' },
			{ id: 'seedream', name: 'Seedream 4' },
			{ id: 'sd35', name: 'Stable Diffusion 3.5' },
			{ id: 'dalle3', name: 'DALL-E 3' },
			{ id: 'midjourney', name: 'Midjourney' },
		]
		const model = MODELS.find(m => m.id === shape.props.node.model)
		if (model) {
			headingText = model.name
		}
	} else if (shape.props.node.type === 'videoGeneration' && 'model' in shape.props.node) {
		const VIDEO_MODELS = [
			{ id: 'runway-gen4', name: 'Runway Gen-4' },
			{ id: 'minimax', name: 'Minimax Video' },
			{ id: 'kling', name: 'Kling' },
			{ id: 'veo3', name: 'Veo 3' },
			{ id: 'luma-ray2', name: 'Luma Ray 2' },
		]
		const model = VIDEO_MODELS.find(m => m.id === shape.props.node.model)
		if (model) {
			headingText = model.name
		}
	}
	...
	return (
		<HTMLContainer
			className={classNames('NodeShape', `NodeShape-${shape.props.node.type}`, {
				NodeShape_executing: isExecuting,
			})}
		>
			<div className="NodeShape-heading">
				<div className="NodeShape-label">{headingText}</div>
				{output !== undefined && (
					<>
