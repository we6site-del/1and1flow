...
def generate_image(prompt, model, aspect_ratio="1:1", extra_params=None, num_images=1):
    ...
    try:
        ...
        if images:
            ...
            if len(images) > 0:
                img_item = images[0]
                # OpenRouter might return: 
                # 1. Object with .image_url.url
                # 2. Dict with ['image_url']['url']
                # 3. Flat dict or object? (less likely for standard chat format)
                ...
                # Try dict access
                elif isinstance(img_item, dict) and 'image_url' in img_item:
                     img_url_obj = img_item['image_url']
        ...
        # Extract Image URL
        # 1. Regex for markdown image: ![...](url)
        import re
        img_match = re.search(r'!\[.*?\]\((.*?)\)', content)
        if img_match:
            return img_match.group(1)
            
        # 2. Regex for direct URL
